...
<!--
  Architecture Template: User Journey
  ...
  Target location: ${DOC_ROOT}/architecture/journeys/[journey-slug].md

  Source: Derived from https://gist.github.com/robertDouglass/e5b53e360fcee01bd9bebd7e33850ebf
  Doctrine alignment:
  - Directive 018 (Traceable Decisions) — decision capture in Design Decisions section
  - Directive 040 (Human-in-Charge Escalation) — actor attribution, human oversight
  - Directive 021 (Locality of Change) — scope boundaries in the Scope section
  - Directive 016 (ATDD) — acceptance scenarios in Given/When/Then format
-->
...
<!--
  ...
  doesn't exist yet, create one using the companion persona template at
  doctrine/templates/architecture/PERSONA.md (full profile: motivations,
  frustrations, behavioral cues, collaboration preferences, success measures).

  Use Persona ID to cross-reference. When an actor's behavior in a journey
  phase seems surprising, the full persona explains why.
-->
...
<!--
  ...
  Adapt column headers to your domain. For example, rename "System" to match
  your boundary (CLI, SaaS, Orchestrator, Backend, etc.).
-->
...
<!--
  ...
  See Directive 016 (ATDD) for acceptance scenario standards.
-->
...
<!--
  ...
  See Directive 018 (Traceable Decisions) for ADR standards.
-->
