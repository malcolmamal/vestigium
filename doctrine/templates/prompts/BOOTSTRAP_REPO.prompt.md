...
3. Set up Cursor IDE integration: run `doctrine/cursor/setup.sh`
   (symlinks hooks, generates skills, creates rules directory).
4. Recommend initial agent set (list existing + suggested removals/additions).
5. Output scaffold diffs (do not overwrite silently) with creation notices.
6. Summarize next 5 refinement actions.
