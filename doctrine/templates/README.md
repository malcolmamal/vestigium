...
This directory contains standardized templates for consistent artifact creation across the Regnology Professional Services Agent Framework. Templates define required structure, sections, and metadata for various document types used in agent-augmented development.
...
├── branding/             Regnology PS ASCII art logos and file headers
...
├── license/              Regnology PS Internal Tooling license and metadata
...
### branding/
**Purpose:** Regnology PS ASCII art logos and source-file headers  
**Key Files:**
- `PS_logo_art` — Full boxed logo with address block (indented)
- `PS_logo_art_small` — Boxed logo without address (flush-left)
- `PS_logo_art_minimal` — Raw logo text, no border
- `header.txt` — Compact file header: small logo + internal-use notice

**Usage:** Repository scaffolding, CLI banners, generated file headers
...
### license/
**Purpose:** Regnology PS Internal Tooling license files  
**Key Files:**
- `LICENSE` — Full proprietary license text with branded header
- `licenses.properties` — Short license identifier mapping
- `regnology-internal.properties` — License metadata (name, URL, distribution)

**Usage:** Copy to new repository roots during scaffolding. See `license/README.md` for details.
...
**Key Files:**
- `REPO_MAP.md` — Repository topology map
- `SURFACES.md` — Dependency and integration surface taxonomy
- `WORKFLOWS.md` — Workflow catalog
- `CODEOWNERS` — Bitbucket Cloud CODEOWNERS template (auto-assigns PR reviewers by path/team)
- `teams.yaml` — Companion ad-hoc team definitions for CODEOWNERS

**Usage:** See [Directive 003 (Repository Quick Reference)](../directives/003_repository_quick_reference.md)  
**CODEOWNERS usage:** Copy `CODEOWNERS` and `teams.yaml` to `.bitbucket/` in the consuming repository. Fill in owners and team members. See [Bitbucket CODEOWNERS docs](https://support.atlassian.com/bitbucket-cloud/docs/set-up-and-use-code-owners/) and `doctrine/tactics/repository-initialization.tactic.md` step 2a.
