...
## Load Repository-Specific Guidelines

When the file exists, load **`doctrine/guidelines/regnology_specific.md`** as part of the bootstrap sequence (after core doctrine and before or alongside `.doctrine-config/` overrides). It defines repository-specific rules (e.g. Regnology branding, model discipline, domain constraints) that agents must follow in this repository.
...
2. Load **`doctrine/guidelines/regnology_specific.md`** when present, then load `.doctrine-config/` overrides after core doctrine loading, if present, and validate non-conflict with General/Operational guidelines.
...
4. **Introduce yourself** per [Directive 012, Section 2](../directives/012_operating_procedures.md#2-identity-confirmation):
    - Emit your Initialization Declaration block (from your agent profile).
    - Begin substantive outputs with `[Agent: <Name>]` throughout the session.
    - Include a `Recommended Next` section at task completion.
5. Perform the first small step:
    - Prefer analysis or planning over large code changes.
    - Use relevant mode aliases before reasoning; keep scratch in `work/`.
6. Stop after a coherent unit of work and summarise:
