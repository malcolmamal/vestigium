...
**Key Principle:** The Assumption Check validates that your test setup created the scenario you intended to test. This catches test setup bugs early and makes test failures more precise.
...
## Validation & Guard Clauses {#validation--guard-clauses}
...
Test suites should follow the Testing Pyramid pattern:
...
```
  ...
  /              \
 /________________\ Unit Tests (50-70%)
/                  \ Fast, isolated, focused
```

### Guidelines

1. **50-70% Unit Tests:** Fast, isolated function/method tests
   - Execution time: <0.5 seconds for full unit suite
   - Each test validates single behavior
   - Use fixtures for test isolation
   - No external dependencies (databases, APIs, files)

2. **20-30% Integration Tests:** Component interaction validation
   - Execution time: <2 seconds for full integration suite
   - Test interactions between modules
   - May use temporary files, in-memory databases
   - Mock external services

3. **10-20% E2E Tests:** Complete workflow validation
   - Execution time: <5 seconds for full E2E suite
...
- Individual unit test: <10ms average
- Individual integration test: <100ms average
- Individual E2E test: <500ms average
- Full test suite: <10 seconds total
- CI/CD execution: <30 seconds with overhead
...
│   └── fixtures/
│       └── sample_data.yaml
├── fixtures/               # Test data (optional alternative)
...
The **Quad-A (Arrange-Assumption-Act-Assert)** pattern extends the traditional AAA pattern with an explicit assumption check phase. This catches test setup failures early and provides more precise test failure messages.

**Origin:** Validated through production test suite development  
**Benefit:** Distinguishes between setup failures and actual test failures  
**Adoption:** Particularly valuable for file-based testing and complex fixture scenarios
...
|---------|------------|----------------------------------------------|
| 1.0.0   | 2026-02-08 | Extracted from saboteurs styleguides (generic patterns only) |

---

**Maintained by:** Regnology Professional Services  
**Review Cycle:** Quarterly or when significant patterns emerge  
**Related:** See `doctrine/guidelines/version-control-hygiene.md` for Git conventions
