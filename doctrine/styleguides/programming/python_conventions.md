...
When accessing resources (fixtures, scripts, config files) inside tests or utilities, prefer computing paths relative to the current file using `Path(__file__).resolve().parents[...]`. This keeps tests IDE-friendly and resilient to changing working directories.
...
- **Quad-A Testing:** `work/notes/ideation/opinionated_platform/opinions/quad_A_test_structure.md`
- **Testing Pyramid:** `work/notes/ideation/opinionated_platform/opinions/testing_pyramid.md`
