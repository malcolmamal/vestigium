...
author = "Regnology Professional Services"
...
### 5.1 Test Data Builders

For complex objects, use the **Test Data Builder** pattern to improve test readability and reduce boilerplate. A builder provides sensible defaults and allows tests to specify only the fields relevant to the test case.

- **GOOD (Using a Test Data Builder):**
  ```java
  // In a test fixture class
  public class ExposureRecordFixtures {
      public static ExposureRecord.Builder aRecord() {
          return ExposureRecord.builder()
              .canonicalId(CanonicalIdentifier.of("REP_TEST_20"))
              .lot("20")
              // ... other defaults
      }
  }
  ...
  void shouldTestSomething() {
      var record = ExposureRecordFixtures.aRecord()
          .exposureClassType("Corporates")
          .build();
      
      // ... assert something about the record
  }
  ```
...
// Prefer Optional for default values over null checks
// While a ternary operator works, `Optional.ofNullable().orElse()` is more explicit
// about the intent to handle potential nullity and is a more idiomatic functional pattern.
// GOOD:
BigDecimal effectiveTolerance = Optional.ofNullable(tolerance).orElse(DEFAULT_TOLERANCE);
// ACCEPTABLE, BUT LESS IDIOMATIC:
BigDecimal effectiveTolerance = tolerance != null ? tolerance : DEFAULT_TOLERANCE;
```
...
### 6.5 Exception Handling and Assertions

When testing for expected exceptions, always assert for the *specific* exception type the code is documented to throw. Avoid catching generic `Exception` or `RuntimeException`.

- **GOOD (Asserting for the specific exception):**
  ```java
  // Test that Objects.requireNonNull throws the documented NullPointerException
  assertThatThrownBy(() -> service.process(null))
      .isInstanceOf(NullPointerException.class);
  ```
- **BAD (Asserting for a generic or incorrect exception):**
  ```java
  // The code throws NullPointerException, but the test expects a different exception.
  // This test would fail and indicates a misunderstanding of the code's contract.
  assertThatThrownBy(() -> service.process(null))
...
Always specify character encoding explicitly when reading from or writing to files. Relying on the system's default encoding can lead to bugs when the application is run on different platforms.

- **GOOD (Explicitly using UTF-8):**
  ```java
  try (var reader = new InputStreamReader(new FileInputStream(path), StandardCharsets.UTF_8)) {
      // ... process reader
  }
  ```
- **BAD (Using default system encoding):**
  ```java
  try (var reader = new FileReader(path)) { // DON'T DO THIS
      // ... this may fail on systems with a non-UTF-8 default
  }
  ```
