...
Clarifies scope: govern initialization, interpretation, and operation inside the contextual environment ensuring behavioral and reasoning integrity.
...
Agents MUST match model tier to task complexity:

- **Match model to arena.** Map tasks to arenas (text, code, vision, search). Routine extraction, file reading, YAML edits, and git operations are text-arena, mid-tier work — not premium.
- **Consult the toolguide.** Use `doctrine/toolguides/agentic_model_leaderboard.yaml` for model rankings by arena and cost tier. Do not hardcode model preferences.
- **Delegate routine tasks** to cheaper/faster models or subagents. Routine = file creation, search, git operations, template generation, structured artifact writing.
- **Reserve the premium thread** for complex reasoning, architecture, trade-off analysis, debugging, and active iteration with the user.
- **Document model choice.** When coordinating tasks, set `context.preferred_execution_model_tier` so downstream agents respect cost-appropriate selection.
- **Self-check:** "Which arena? Which tier? Does this require my full reasoning depth?" If not, delegate.
- **Batch delegations** — send multiple independent subagent tasks in a single message for parallelism.
- **Log model choice** when it deviates from the recommended tier (Directive 018).

Reference: `doctrine/directives/042_model_discipline.md`, `doctrine/tactics/model-discipline-selection.tactic.md`
...
**Purpose:** Tool-specific distribution of doctrine content for Cursor IDE. Generated from `doctrine/` sources — edit doctrine, not `.cursor/` directly.

**Structure:**
- `.cursor/rules/` - Glob-scoped editing rules and quick-reference
- `.cursor/skills/` - On-demand skill prompts (approaches, tactics, conventions)
- `.cursor/hooks.json` - Validation and advisory hooks
- `.cursor/hooks/` - Hook implementation scripts

**See:** `doctrine/agents/curator.agent.md` § 2.1 for the full export/deploy pipeline.
...
**Terminology Reference:** See [doctrine/glossary/](doctrine/glossary/) for standardized definitions of terms used throughout this framework. The legacy flat glossary is preserved at [doctrine/GLOSSARY.md](doctrine/GLOSSARY.md) for historical reference.
...
| 007  | [Agent Declaration & Self-Introduction](doctrine/directives/007_agent_declaration.md) | Mandatory operational authority affirmation and user-facing self-introduction |
...
| 014  | [Work Log Creation](doctrine/directives/014_worklog_creation.md)             | Standards for work logs with token count and context metrics    |
| 015  | [Store Prompts](doctrine/directives/015_store_prompts.md)                 | Optional prompt documentation with SWOT analysis for improvement|
| 016  | [Acceptance Test Driven Dev](doctrine/directives/016_acceptance_test_driven_development.md)    | ATDD workflow and acceptance test requirements (ADR-012)        |
| 017  | [Test Driven Development](doctrine/directives/017_test_driven_development.md)       | TDD workflow and unit test requirements (ADR-012)               |
| 018  | [Traceable Decisions](doctrine/directives/018_traceable_decisions.md)           | Decision capture protocols and traceability (ADR-017)           |
| 019  | [File-Based Collaboration](doctrine/directives/019_file_based_collaboration.md)      | Multi-agent orchestration through file-based task workflows     |
...
| 034  | [Specification-Driven Development](doctrine/directives/034_spec_driven_development.md) | Functional requirement capture before implementation (recommended, optional)|
...
| 037  | [Context-Aware Design](doctrine/directives/037_context_aware_design.md) | Design decisions informed by bounded context and system landscape |
| 038  | [Ensure Conceptual Alignment](doctrine/directives/038_ensure_conceptual_alignment.md) | Confirm shared understanding of domain terminology before task execution (mandatory) |
| 039  | [Refactoring Techniques](doctrine/directives/039_refactoring_techniques.md) | Safe, incremental code structure improvements via test-backed tactics |
| 040  | [Human-in-Charge Escalation](doctrine/directives/040_human_in_charge_escalation_protocol.md) | Structured agent-to-human escalation via work/human-in-charge/ (mandatory) |
| 041  | [Use  Branding](doctrine/directives/041_use__branding.md) | Mandatory branding headers and licensing for all  PS repositories |
| 042  | [Model Discipline](doctrine/directives/042_model_discipline.md) | LLM model selection by task arena and cost tier; prefer high-ROI defaults |
| 043  | [AppSec Compliance](doctrine/directives/043_appsec_compliance.md) | Mandatory security controls, gates, coding standards, and SBOM for all deliverable software |
| 043  | [Use Corporate Tooling](doctrine/directives/043_use_corporate_tooling.md) | Standards for Jira, Confluence, and corporate tool interactions |

Location: `doctrine/directives/XXX_name.md` (42 directives). Example load pattern:
